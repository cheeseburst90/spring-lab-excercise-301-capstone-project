...
import com.eatza.order.dto.*;
import com.eatza.order.exception.OrderException;
import com.eatza.order.model.CommiqueForm;
...
public class OrderServiceImpl implements OrderService {
    ...
    public Order placeOrder(OrderRequestDto orderRequest) throws OrderException {
        logger.info("In place order method, creating order object to persist");
        ...
        logger.info("Getting all ordered items to persist");
        ...
        for (OrderedItemsDto itemDto : itemsDtoList) {
            ...
            try {
                logger.info("Calling restaurant search service to get item details with itemid: " + itemDto.getItemId());
                String finalRestaurantUrl = restaurantServiceItemUrl + itemDto.getItemId();
                logger.info("finalRestaurantUrl: " + finalRestaurantUrl);
                ItemFetchDto item = restTemplate.getForObject(finalRestaurantUrl, ItemFetchDto.class);
                ...
                CommiqueForm commiqueForm = new CommiqueForm("order created", "from order service");
    ...
    public boolean cancelOrder(Long orderId) {
        logger.info("In cancel order service method, calling repository");
        Optional<Order> order = orderRepository.findById(orderId);
        if (order.isPresent()) {
            logger.info("Order was found in db");
